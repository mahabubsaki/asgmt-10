...
const Checkoutform = ({ user }) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        toast.success("Thanks for ordering,We will call you as soon as possible", toastSettings)
        e.target.reset();
    }
    return (
        <div className="w-75">
            <form onSubmit={handleSubmit}>
                {
                    displayName &&
                    <input type="text" className="w-100 mx-auto d-block py-2 px-4 border-0 mb-2" placeholder="Name" style={{ backgroundColor: '#F5F5F5' }} readOnly value={displayName} />
                }
                {
                    (!displayName && emailVerified) &&
                    <input type="text" className="w-100 mx-auto d-block py-2 px-4 border-0 mb-2" placeholder="Name" style={{ backgroundColor: '#F5F5F5' }} required />
                }
